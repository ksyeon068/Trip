...
import "../styles/ExploreDetail.scss";
import { translateCategories } from "../utils/categoryTranslate";
...
const ExploreDetail = () => {
  ...
  // 🔵 이미지 가져오기 (Wikipedia → Unsplash → 기본이미지)
  useEffect(() => {
    if (!place) return;
  ...
  const name =
      place.properties.name_international?.ko ||
  ...
  // ⭐ 여기서 번역 생성 (순서 중요)
  const categoryList = translateCategories(categories || []);
    ...
    <div className="explore-detail">
      ...
      <ul className="info">
        ...
        {website && (
          <li>
            <strong>웹사이트</strong>
            <a href={website} target="_blank" rel="noopener noreferrer" className="weblink">
              {website}
            </a>
          </li>
        )}
